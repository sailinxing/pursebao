    ...
    <div class="logo_nav warp">
        <h1 class="logo left" title="钱贷宝 | 你的钱袋是个宝">
            <a href="/" title="钱贷宝 | 你的钱袋是个宝">
                <img src="${pageContext.request.contextPath}/static/picture/logo_1.png" align="钱贷宝 | 你的钱袋是个宝"/>
                <span>|&nbsp;&nbsp;你的钱袋是个宝</span>
            </a>
        </h1>
        <div class="nav right">
...
<div class="invesMainWrap">
    <div class="invesMain clear">
            ...
            <div class="left">
                <div class="introduceText">
                    ...
                    <ul class="list">
                        ...
                        <li><p class="num"><em>${findproduct.loanChildInfo.loanLimittime}</em>天</p>
                            <p class="label">产品期限</p></li>
                        <li><p class="num"><em>${findproduct.startMoney}</em>元</p>
                    ...
                    <div class="introduceText_footer">
                        <span>产品规模：<em>${findproduct.loanChildInfo.loanAmount/10000}万</em></span>
                    </div>
                </div>
            </div>
            <div class="inright">
                <div class="price">剩余可投：<em>${findproduct.surplusMoney}</em>元</div>
                ...
                <div class="pay">
                    <input type="text" value="" name="investMoney" placeholder="${findproduct.startMoney}元起投"/>
                </div>
                <div class="payBtn">
        ...
        <div class="productBox  productInfo">
            ...
            <ul>
                <li>
                    <img src="${pageContext.request.contextPath}/static/picture/bg1.jpg" alt=""/>
                    <!--1：散标 2：金交所 3：活期宝-->
                    <p> 产品通过风控系统审核
                        <span>${findproduct.strCheackDate}</span></p>
                </li>
                <li>
                    <img src="${pageContext.request.contextPath}/static/picture/bg2.jpg" alt=""/>
                    <p>产品正式在金联储金服发布 <span>${findproduct.strPublishDate}</span></p>
                </li>
                <li>
                    <img src="${pageContext.request.contextPath}/static/picture/bg3.jpg" alt=""/>
                    <p>产品还款日期 <span>${findproduct.strRepaymentDate}</span></p>
                </li>
            </ul>
        </div>
        <div class="productBox productIntro">
            ...
            <p style="word-break:break-all;">
                ${findproduct.productIntroduce}
                该产品是投资人（委托人）通过平台自愿将合法所有的资金委托资产管理机构对特定项目进行投资，受托资产管理机构以委托人利益最大化为原则对定向委托投资资金按约定进行定向投资管理，以获取投资收益。本项目暂不对投资人（委托人）收取管理费。</p>
            <h5>融资方信息</h5>
            <p style="word-break:break-all;">
                基本信息<br>借款企业${findproduct.loanChildInfo.userChildInfo.companyInfo.companyName}。<br>涉诉情况<br>${findproduct.loanChildInfo.userChildInfo.companyInfo.caseComplaint}<br>根据全国法院被执行人信息查询系统及互联网搜索，未发现借款企业不良涉诉信息。<br>资金用途<br>用于企业经营，补充流动资金。<br>还款来源<br>企业经营性收入。
            </p>
            <h5>增信措施</h5>
            <p style="word-break:break-all;">
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0" class="invoteList">
                    ...
                    <%--<c:forEach items="${investList}" var="invest">
                        <tr class="tab_bg">
                            ...
                            <td class="f_money">${invest.investMoney}</td>
                            <td>${invest.strInvestDate}</td>
                        </tr>
                    </c:forEach>--%>
...
<%--投资历史记录分页--%>
<script type='text/javascript'>
    var pid = "${findproduct.pid}";
        $.ajax({
            type: "GET",
            url: "${pageContext.request.contextPath}/invest/getcount",
            data:{"pid":pid},
            async:false,
    ...
    function getFirstRecode() {
        $.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/invest/getrecode",
            async: false,
            data: {"pid": pid, "currentpage": 1},
            success: function (data) {
                listRecode(data);
            }
        });
    ...
    function getRecodePage(cpage) {
        $.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/invest/getrecode",
            data: {"pid": pid, "currentpage": cpage},
            success: function (data) {
                listRecode(data);
            }
        });
    ...
    function listRecode(recodedata) {
        var html = "";
        html+= '<tr class="tab_bg">';
        html+= '<th width="30%">投资人</th>';
        html+=  ' <th width="30%">投资金额（元）</th>';
        html+= '<th width="30%">投资时间</th>';
        html+=  '</tr>';
        for (var i = 0; i < recodedata.length; i++) {
            var recode = recodedata[i];

            html+='<tr class="tab_bg">';
            html += '<td>' + recode.username + '</td>';
            html += '<td class="f_money">' + recode.investMoney + '</td>';
            html += '<td>' + recode.strInvestDate + '</td>';
            html+='</tr>';
        }
        $("#recodelist").empty();
        $("#recodelist").html(html);
...
<script type='text/javascript'>
    ...
    $(function () {
        $(document).on("click", "#goInvest", function () {
            var amount = parseInt($("input[name=investMoney]").val());
            var residue = ${findproduct.surplusMoney};
            var begin = ${findproduct.startMoney};
            var pid = "${findproduct.pid}";

            /* var id = "111";
             var projectAccount = "0";
             var isLogin = true;
             var isNewOnly = parseInt("1");
             var blance = parseFloat("0".replace(/,/g, ''));
             if (isNewOnly && isLogin && (projectAccount > 0)) {
                 newPlan();
                 return false;
             }*/
            if (!amount) {
                $(".tips").show().html("投资金额不能为空");
                return false;
            }
            if (amount > residue) {
                $(".tips").show().html("投资金额不能大于可投金额");
                return false;
            }
            if (amount < begin) {
                $(".tips").show().html("投资金额不能小于起投金额");
                return false;
            }
            /*if ((amount > blance) && isLogin) {
                $(".tips").show().html("可用余额不足，请先充值");
                return false;
            }*/


            location.href = "${pageContext.request.contextPath}/invest/investorder/" + amount + "/" + pid;
...
<div class="footer">
    <div class="footer_nav warp">
        ...
        <div class="footer_codeimg left">
            <span><img src="${pageContext.request.contextPath}/static/picture/u20_1.png"/><br/>关注金联储金服微信</span>
            <span><img src="${pageContext.request.contextPath}/static/picture/u25_1.png"/><br/>下载金联储金服APP</span>
        </div>
        ...
        <p>
            <a id="_pingansec_bottomimagelarge_p2p" target="_blank" title="互联网金融品牌"
               href="http://si.trustutn.org/info?sn=448170824000601194838&certType=4"><img
                    src="${pageContext.request.contextPath}/static/picture/footer_img5_1.jpg"/></a>
            <a href="http://www.bjp2p.com.cn/" target="_blank" title="北京市网贷行业协会发起会员"><img
                    src="${pageContext.request.contextPath}/static/picture/footer_img1_1.jpg"/></a>
            <a href="http://v.pinpaibao.com.cn/authenticate/cert/?site=www.jinlianchu.com&at=business" target="_blank"
               title="安全联盟认证"><img src="${pageContext.request.contextPath}/static/picture/footer_img2_1.jpg"/></a>
            <a href="https://credit.szfw.org/CX20170913036162381669.html" target="_blank" title="诚信网站"><img
                    src="${pageContext.request.contextPath}/static/picture/footer_img3_1.jpg"/></a>
        </p>
    </div>
