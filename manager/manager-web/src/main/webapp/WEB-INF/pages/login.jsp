...
<div class="login">
    ...
    <form action="${pageContext.request.contextPath}/index" onsubmit="return formAction()" method="post">
        ...
        <button type="submit" class="btn btn-primary btn-block btn-large" accesskey="enter">登录</button>
    </form>
</div>
...
<script type="text/javascript">
    ...
    function formAction() {
        ...
        $.ajax({
            type:"POST",
            url:"${pageContext.request.contextPath}/begin/login",
            async:false,
            cache:false,
            dataType:"text",
            data:{
                "managerName":$("#username").val(), "password":$("#password").val()
            },
            success:function(data){
                if (data == "success") {
                    <%--location.href="${pageContext.request.contextPath}/usermanagement/users";--%>
                } else {
                    alert("用户名或密码错误!");
                    isSubmit = false;
                }
